...
public class RoleConfigsExcelElementItem<T extends List<PafPlannerConfig>> extends PafExcelElementItem<T> {
	...
	protected void createHeaderListMapEntries() {
		...
		getHeaderListMap().put(getSheetName(), Arrays.asList("role name", "plan cycle name", "default eval on working version", "default eval ref versions (pipe delimited)", "save working version on uow load", "default rule set name", "rule set names (pipe delimited)", "read only measures (pipe delimited)", "version filters (pipe delimited)", "enable role filter", "enable multi-select role filter", "role filter (pipe delimited)", "view / view group names (pipe delimited)", "visible menus (pipe delimited)", "auto save menus (pipe delimited)", "replicate enabled", "replicate all enabled",  "lift enabled", "lift all enabled", "allow suppress invalid intersections", "suppress invalid intersection entrys (pipe delimited)", "large cell limit", "max cell limit"));
			...
			for ( Integer rowIndex : row.getRowItemOrderedIndexes()) {
				...
				try {
					...
					switch (rowIndex) {
						...
						//default eval ref versions
						case 3:
							
							roleConfig.setDefaultEvalRefVersions(PafExcelUtil.getStringArFromDelimValueObject(getProjectElementId(), firstValueObject));
							break;
	...
	protected void writeExcelSheet(T t) throws PaceProjectWriteException, PafException {
		...
		if ( roleConfigList != null ) {
			...
			for ( PafPlannerConfig plannerConfig : roleConfigList ) {
				...
				//default eval ref versions
				String[] refVersions = plannerConfig.getDefaultEvalRefVersions();
				if ( refVersions != null && refVersions.length > 0 ) {
					...
					for ( String versionItem : refVersions ) {
						if ( versionsRefMap != null && versionsRefMap.containsKey(versionItem) ) {
							
							valueObjectList.add(PafExcelValueObject.createFromFormulaReferenceMap(versionItem, versionsRefMap));
							
						} else {
							
							valueObjectList.add(PafExcelValueObject.createFromString(versionItem));
					...
					excelRow.addRowItem(3, PafExcelUtil.getDelimValueObjectFromList(valueObjectList));
					
				}	

				//mdbSaveWorkingVersionOnUowLoad
				excelRow.addRowItem(4, PafExcelValueObject.createFromBoolean(plannerConfig.isMdbSaveWorkingVersionOnUowLoad()));
				
				//defaultRulesetName
				excelRow.addRowItem(5, PafExcelValueObject.createFromFormulaReferenceMap(plannerConfig.getDefaultRulesetName(), dynamicRefProjectElementMap.get(ProjectElementId.RuleSets)));
								
				//ruleSetNames
				String[] ruleSetNames = plannerConfig.getRuleSetNames();
				if ( ruleSetNames != null && ruleSetNames.length > 0 ) {
												
					valueObjectList = PafExcelValueObjectUtil.createListOfDynamicReferencePafExcelValueObjects(ruleSetNames, dynamicRefProjectElementMap.get(ProjectElementId.RuleSets));
					
					excelRow.addRowItem(6, PafExcelUtil.getDelimValueObjectFromList(valueObjectList));
				}	
				
				//read only measures
				String[] roMeasures = plannerConfig.getReadOnlyMeasures();
				if ( roMeasures != null && roMeasures.length > 0 ) {
					
					valueObjectList = new ArrayList<PafExcelValueObject>();
					
					Map<String, String> measuresRefMap = dynamicRefProjectElementMap.get(ProjectElementId.Measures);
					
					for (String measureName : roMeasures) {
				...
				//version filter
				String[] versionFilter = plannerConfig.getVersionFilter();
				if ( versionFilter != null && versionFilter.length > 0 ) {
					...
					for ( String versionFilterItem : versionFilter ) {
					...
					excelRow.addRowItem(8, PafExcelUtil.getDelimValueObjectFromList(valueObjectList));
					
				}	
				
				//isUserFilteredUow
				excelRow.addRowItem(9, PafExcelValueObject.createFromBoolean(plannerConfig.getIsUserFilteredUow()));
				
				//isUserFilteredMultiSelect
				excelRow.addRowItem(10, PafExcelValueObject.createFromBoolean(plannerConfig.getIsUserFilteredMultiSelect()));
				
				//user filter spec
				UserFilterSpec userFilterSpec = plannerConfig.getUserFilterSpec();
				if ( userFilterSpec != null ) {
				
					excelRow.addRowItem(11, PafExcelUtil.getDelimValueObjectFromStringAr(userFilterSpec.getAttrDimNames()));
												
				}
				
				//view tree item names
				String[] viewTreeItems = plannerConfig.getViewTreeItemNames();
				if ( viewTreeItems != null && viewTreeItems.length > 0 ) {
					...
					for ( String viewTreeItemName : viewTreeItems ) {
				...
				//menu item names
				String[] menuItemNames = plannerConfig.getMenuItemNames();
				if ( menuItemNames != null && menuItemNames.length > 0 ) {
												
					valueObjectList = PafExcelValueObjectUtil.createListOfDynamicReferencePafExcelValueObjects(menuItemNames, dynamicRefProjectElementMap.get(ProjectElementId.CustomMenus));
				...
				String[] autoSaveMenuItem = plannerConfig.getAutoRunOnSaveMenuItemNames();
				if ( autoSaveMenuItem != null && autoSaveMenuItem.length > 0 ) {
												
					valueObjectList = PafExcelValueObjectUtil.createListOfDynamicReferencePafExcelValueObjects(autoSaveMenuItem, dynamicRefProjectElementMap.get(ProjectElementId.CustomMenus));
					
					excelRow.addRowItem(14, PafExcelUtil.getDelimValueObjectFromList(valueObjectList));
					
				}	
				
				//replicate enabled
				excelRow.addRowItem(15, PafExcelValueObject.createFromBoolean(plannerConfig.getReplicateEnabled()));
				
				//replicate all enabled
				excelRow.addRowItem(16, PafExcelValueObject.createFromBoolean(plannerConfig.getReplicateAllEnabled()));
				
				//lift enabled
				excelRow.addRowItem(17, PafExcelValueObject.createFromBoolean(plannerConfig.getLiftEnabled()));
				
				//lift all enabled
				excelRow.addRowItem(18, PafExcelValueObject.createFromBoolean(plannerConfig.getLiftAllEnabled()));
				//is data filtered uow
				excelRow.addRowItem(19, PafExcelValueObject.createFromBoolean(plannerConfig.getIsDataFilteredUow()));
				
				//data filter spec
				DataFilterSpec dateFilterSpec = plannerConfig.getDataFilterSpec();
				if ( dateFilterSpec != null ) {
					PafDimSpec[] pafDimSpecAr =  dateFilterSpec.getDimSpec();
					if( pafDimSpecAr != null && pafDimSpecAr.length > 0 ) {
						...
						excelRow.addRowItem(20, PafExcelValueObject.createFromString(strBuff.toString()));					
					}
				}
				
				//uow size large
				excelRow.addRowItem(21, PafExcelValueObject.createFromInteger(plannerConfig.getUowSizeLarge()));
								
				//uow size max				
				excelRow.addRowItem(22, PafExcelValueObject.createFromInteger(plannerConfig.getUowSizeMax()));
