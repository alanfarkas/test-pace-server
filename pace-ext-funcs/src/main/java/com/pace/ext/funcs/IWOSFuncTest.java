 ...
 *
 */
public class IWOSFuncTest extends TestCase {
...
//			String[] mbrs = {"WP", "ClassChn", "WK05", "DPT110", "FWOS", "Store1", "FY2005"};
			...
			sourceIs.setCoordinate(measureDim, "BOPRTL_DLR");
			...
			PafBaseTree timeTree = esbMetaData.getBaseDimension(timeDim);
			...
			try {
				...
				result = iwosFunc.calculate(sourceIs, dataCache, evalState);
				isSuccess = false;
			...
			try {
				...
				args = new String[1];
				args[0] = salesMeas;
				iwosFunc.parseParameters(StringUtils.arrayToString(args, "(", ")", "", "", ","));
				result = iwosFunc.calculate(sourceIs, dataCache, evalState);
				isSuccess = false;
			} catch (PafException pfe) {
			} finally {
			...
			try {
				...
				iwosFunc.parseParameters(StringUtils.arrayToString(args, "(", ")", "", "", ","));
				result = iwosFunc.calculate(sourceIsQ1, dataCache, evalState);
