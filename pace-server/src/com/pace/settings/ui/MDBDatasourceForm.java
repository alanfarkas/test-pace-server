...
public class MDBDatasourceForm extends PaceSettingsForm implements Property.ValueChangeListener {
	...
	protected void setupForm() {
		...
		setPaceSettingsDefaultFactory(serverSettingsApplication.getMdbDatasourceFieldFactory());
	...
	protected void saveForm() {
		...
		if ( newMdbDatasource ) {
			...
			serverSettingsApplication.getMdbDatasourceContainer().addItem(pafConnectionProps);
			
			setItemDataSource(null);
												
			serverSettingsApplication.getMdbDatasourceTable().select(pafConnectionProps);
	...
	public void setItemDataSource(Item newDataSource) {
		...
		if ( newDataSource != null ) {
			super.setItemDataSource(newDataSource, serverSettingsApplication.getMdbDatasourceFieldFactory().getFormOrderList());
			getFooter().setVisible(true);
		
			if ( newDataSource instanceof BeanItem ) {
				pafConnectionProps = (PafConnectionProps) ((BeanItem) newDataSource).getBean();
			}
	...
	public boolean isValid() {
		...
		if ( isValid) {
			...
			String newName = (String) getField(MDBDatasourceFieldFactory.NAME).getValue();
			
			boolean nameExists = false;
			
			for ( PafConnectionProps pafConnectionProps : serverSettingsApplication.getMdbDatasourceContainer().getItemIds() ) {
				
				if ( pafConnectionProps.getName() != null && pafConnectionProps.getName().trim().equals(newName) ) {
			...
			if ( nameExists && newMdbDatasource ) {
				...
				isValidNotificationOverrideError = "Name " + newName + " already exists.  Please choose a different name.";
				isValid = false;
			...
			String connectionString = (String) getField(MDBDatasourceFieldFactory.CONNECTION_STRING).getValue();
			...
			try {
				...
				isValidNotificationOverrideError = getField(MDBDatasourceFieldFactory.CONNECTION_STRING).getCaption() + " content is invalid.  Content needs to be in key=value form seperated by ';'."; 
				isValid = false;
	...
	public void copyMDBDatasource() {		
		...
		if ( serverSettingsApplication.getMdbDatasourceTable().getValue() != null ) {
			...
			PafConnectionProps selectedConnectionProps = serverSettingsApplication.getMdbDatasourceContainer().getItem(serverSettingsApplication.getMdbDatasourceTable().getValue()).getBean();
